...
interface Props {
	cierreExpiracion: ExpiracionSesion
	idOrg: number
}

export const MensajeCierre = ({ cierreExpiracion, idOrg }: Props) => {
		...
		const params: PostExpiracionSesionParams = {
			...
			payload: {
				idSessionExpirationType:
					cierreExpiracion.idSessionExpirationType,
			},
		}
	...
	useEffect(() => {
		setMensaje(cierreExpiracion.clientMessage)
		setTime(cierreExpiracion.closingTime)
	}, [cierreExpiracion])
		...
		<>
			<Grid item xs={12}>
				<Typography variant="h6">
					Tiempo Inactividad Cliente Cierre
				</Typography>
			</Grid>
			...
			<Grid item xs="auto">
				<Button
					...
					onClick={() => handleClick()}
					disabled={
						mensaje === cierreExpiracion.clientMessage ||
			...
			<Grid item xs="auto">
				<Button
					...
					disabled={time === cierreExpiracion.closingTime}
				>
					Actualizar
				</Button>
			</Grid>
